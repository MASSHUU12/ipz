...
test.describe('Testy strony rejestracji', () => {
    ...
    // dwa dodatkowe testy wyłącznie dla pull request

    test('Rejestracja z poprawnymi danymi dla logowania', async ({ page }) => {
    ...
    test('Rejestracja z długim loginem i hasłem dla logowania', async ({ page }) => {
        const longString = 'a'.repeat(50);

        await page.getByRole('textbox', { name: 'Email or Phone' }).fill(longString + '@testowy.com');
        await page.getByRole('textbox', { name: 'Password', exact: true }).fill('A' + longString + '1!');
        await page.getByRole('textbox', { name: 'Confirm Password' }).fill('A' + longString + '1!');
