  ...
  const getCityLatLng = (city: string): { lat: number; lng: number } | undefined => {
        ...
        <Grid container spacing={3} alignItems="stretch"> 
                  ...
                  <Grid item xs={12} md={6}>
                    <Box sx={{ height: "80%" }}>
                      <Card sx={{ height: "90%", display: "flex", flexDirection: "column", backgroundColor: "#222", color: "#fff", borderRadius: 2, p: 2 }}>
                        <CardContent sx={{ flexGrow: 1 }}>
                          <Typography variant="h6" sx={{ mb: 2 }}>Mapa</Typography>
                          <Box sx={{ height: "100%", borderRadius: 2, overflow: "hidden" }}>
                            <MapContainer center={[52.2297, 21.0122]} zoom={13} style={{ height: "100%", width: "100%" }}>
                              ...
                              />
                              <Marker position={[53.4285,14.5528]}>
                                <Popup>Szczecin</Popup>
                              </Marker>
                            </MapContainer>
                          </Box>
                        </CardContent>
          ...
          <Box sx={{ backgroundColor: "#1e1e1e", boxShadow: 5, p: 2, borderRadius: 2 }}>
            <Grid container spacing={3}>
                ...
                <Card sx={{ height: "80%", backgroundColor: "#222", color: "#fff", borderRadius: 2, p: 2 }}>
                  <CardContent sx={{ flexGrow: 1 }}>
                    ...
                    {weatherLoading || !weather ? (
                      ...
                      <Box>
                        <Typography variant="body1" sx={{ mb: 1 }}>💧 Humidity: {weather.humidity}%</Typography>
                        <Typography variant="body1" sx={{ mb: 1 }}>💨 Wind: {weather.wind_speed} m/s</Typography>
              ...
              <Grid item xs={12} md={6}>
                <Card sx={{ height: "90%", display: "flex", flexDirection: "column", backgroundColor: "#222", color: "#fff", borderRadius: 2 }}>
                  <CardContent sx={{ flexGrow: 1 }}>
                    <Box sx={{ height: "100%", borderRadius: 2, overflow: "hidden" }}>
                      <MapContainer center={getCityLatLng(selectedCity)} zoom={13} style={{ height: "100%", width: "100%" }}>
                        <TileLayer
                          attribution='&copy; <a href="https://openstreetmap.org">OpenStreetMap</a> contributors'
                          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                        />
              ...
              <Grid item xs={12} md={6}>
                <Card sx={{ height: "80%", backgroundColor: "#222", color: "#fff", borderRadius: 2, p: 2 }}>
                  <CardContent sx={{ flexGrow: 1 }}>
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart data={airPollutionChartData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#444" />
                        <XAxis dataKey="name" stroke="#ccc" interval={0} tick={{ fontSize: 12 }} />
                        <YAxis stroke="#ccc" />
                        <Tooltip
                          ...
                          itemStyle={{ color: "#fff" }}
                        />
                        <Bar dataKey="value" radius={[8, 8, 0, 0]} label={{ position: "top", fill: "#fff", fontSize: 12 }}>
